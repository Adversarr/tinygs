...
def load_camera_intrinsics(file_path: str) -> CameraIntrinsic:
    ...
    with open(file_path, "r") as f:
        cam_w = cam_h = fx = fy = cx = cy = k1 = k2 = k3 = p1 = p2 = None
        for line in f:
            line = line.strip()
            ...
            if model.upper().startswith("PINHOLE"):
                ...
                raise RuntimeError(
                    f"Unsupported camera model '{model}' or insufficient params"
                )
            break

    if None in (cam_w, cam_h, fx, fy, cx, cy, k1, k2, k3, p1, p2):
...
def load_camera_extrinsics(
    file_path: str,
) -> Tuple[List[CameraExtrinsic], np.ndarray, np.ndarray]:
...
def project_points_to_camera(
    points: np.ndarray, camera_intrinsics: CameraIntrinsic, T_wc: np.ndarray
    ...
    Returns:
        Tuple of (u coordinates, v coordinates, z depths, unified_mask) for points
    ...
    mask_in_img = (
        (u >= 0)
        & (u < camera_intrinsics.width)
        & (v >= 0)
        & (v < camera_intrinsics.height)
    )
    ...
    return u_in, v_in, z_in, unified_mask
...
def apply_transformation(points: np.ndarray, T: np.ndarray) -> np.ndarray:
    ...
    points_hom = np.hstack(
        (points, np.ones((N, 1), dtype=points.dtype))
    )  # Convert to homogeneous coordinates
    points_transformed_hom = (T @ points_hom.T).T  # Apply transformation
    points_transformed = (
        points_transformed_hom[:, 0:3] / points_transformed_hom[:, 3:4]
